                    ...
                    <div className="container py-2 md:mx-14">
                        <div className="flex flex-wrap ">
                            {Object.keys(products).map((item) => {
                                return <>
                                    <div className="p-2 md:w-1/4 w-full hover:bg-red-300 rounded-md duration-1000">
                                        <Link key={item._id} href={`/slug/${products[item].slug}`}>
...
export async function getServerSideProps() {
    let products = await Product.find({ types: 'biography' })
    return {
